...
log = logging.getLogger("altosybioagents.api.mcp")
...
class MCPAPI(BaseAPI):
    ...
    def pick_mcp_server_folder(self, *, folder_path: str = "",
                                overwrite: bool = False) -> dict:
        """Validate ``folder_path`` and copy it under mcp_servers_dir().

        The Electron renderer presents the native folder picker via
        window.electronAPI.selectFolder(); the chosen path is then handed to
        this method, which performs the actual ingest and registry refresh.

        Returns ``{"ok": True, "server_id": ..., "name": ..., "overwritten": bool}``
        on success, ``{"ok": False, "cancelled": True}`` if no path was
        supplied, or ``{"ok": False, "error": str,
        "needs_overwrite_confirm": bool}`` on a validation/copy failure.
        """
        if not folder_path:
            return {"ok": False, "cancelled": True}
        chosen = Path(folder_path)
