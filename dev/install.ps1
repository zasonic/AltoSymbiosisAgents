...
try {
    ...
    if ((Get-NodeMajor) -lt $NODE_MIN_MAJOR) {
        Install-Node
        if ((Get-NodeMajor) -lt $NODE_MIN_MAJOR) {
            throw "Node $NODE_MIN_MAJOR+ is still missing. Restart your terminal and rerun Start.bat."
        }
    } else {
    ...
    if (-not $py) {
        ...
        $py = Get-PythonExe
        if (-not $py) {
            throw "Python 3.12+ is still missing. Restart your terminal and rerun Start.bat."
        }
    } else {
    ...
    Write-Host "  * Start.bat                - start the app with hot-reload"
    Write-Host "  * dev\build-installer.bat  - build the Windows installer"
    Write-Host ""
} catch {
    Write-Host ""
