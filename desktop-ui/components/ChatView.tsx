...
//
// The pipeline chips above cover one row per specialist step. Everything else
// the backend emits — checkpoint rollbacks, retries, the adversarial
...
type ThinkingRowState = "info" | "ok" | "warn" | "error";
...
function _deriveThinkingTimeline(
  events: { type: string; data: unknown; at: number }[],
): ThinkingRow[] {
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    if (evt.type === "route_decided") {
      ...
      if (r.reasoning) detailParts.push(_truncate(r.reasoning, 80));
    ...
    if (evt.type === "checkpoint_state") {
      ...
      rows.push({
        ...
        detail: r.reason ? _truncate(r.reason, 140) : undefined,
      });
      continue;
    }
    ...
    if (evt.type === "camel_complete") {
      ...
      rows.push({
        ...
        detail: errored ? _truncate(r.error || "", 140) : (parts.join(" · ") || undefined),
      });
      continue;
    }
    ...
    if (evt.type === "reader_complete") {
      ...
      rows.push({
        ...
        detail: detailParts.join(" · ") || (r.intent ? _truncate(r.intent, 100) : undefined),
      });
      continue;
    }
    ...
    if (evt.type === "reasoning_complete") {
      ...
      rows.push({
        ...
        detail: r.detail || (r.thinking_preview ? _truncate(r.thinking_preview, 140) : undefined),
      });
      continue;
    }
...
export function ChatView() {
  ...
  const thinkingTimeline = useMemo<ThinkingRow[]>(
    () => (streamingEvents ? _deriveThinkingTimeline(streamingEvents) : []),
    [streamingEvents],
  );
...
//
// Renders one row per non-chip lifecycle event (see _deriveThinkingTimeline).
